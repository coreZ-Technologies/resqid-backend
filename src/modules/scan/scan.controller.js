...
export const handleScan = asyncHandler(async (req, res) => {
  ...
  // Redirect to emergency profile page on success
  if (result.success && result.student) {
    return res.redirect(`/emergency/profile/${result.student.id}`);
  }
  
  // Return error page or JSON
  res.status(result.result === 'SUCCESS' ? 200 : 400).json(ApiResponse.success('Scan processed', result));
});

// ─── Scan Logs endpoints (new) ───────────────────────────────────────────────
